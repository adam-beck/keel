...
if ! grep -q "keel" README.md compose.yml Caddyfile mprocs.yaml scripts/setup-certs.sh apps/backend/src/db.ts apps/backend/drizzle.config.ts 2>/dev/null; then
...
replace apps/backend/src/db.ts "s/PGUSER \?\? 'keel_admin'/PGUSER ?? '${db_user}'/"
replace apps/backend/src/db.ts "s/PGDATABASE \?\? 'keel'/PGDATABASE ?? '${db_name}'/"
replace apps/backend/drizzle.config.ts "s/PGUSER \?\? 'keel_admin'/PGUSER ?? '${db_user}'/"
replace apps/backend/drizzle.config.ts "s/PGDATABASE \?\? 'keel'/PGDATABASE ?? '${db_name}'/"
